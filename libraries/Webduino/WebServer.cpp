...
WebServer::WebServer(const char *urlPrefix, uint16_t port) :
		m_server(port), m_client(), m_urlPrefix(urlPrefix), m_pushbackDepth(0), m_contentLength(0), m_failureCmd(
				&defaultFailCmd), m_defaultCmd(&defaultFailCmd), m_cmdCount(0), m_urlPathCmd(NULL), m_bufFill(0) {
}
...
void WebServer::outputCheckboxOrRadio(const char *element, const char *name, const char *val, const char *label,
		bool selected) {
